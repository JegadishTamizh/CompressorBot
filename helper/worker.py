...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -preset fast -c:v libx265 -crf 25.5 -map 0:v -c:a libopus -b:a 70k -ac 2 -map 0:a -c:s copy -map 0:s? -metadata title="Tamil Fusion | t.me/TamilFusion1" -metadata "Encoded By"="TxR Encodes | t.me/TamilFusion1" -metadata:s:v title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:a title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:s title="Tamil Fusion | t.me/TamilFusion1" "{out}" -y'
        process = await asyncio.create_subprocess_shell(
...
async def customenc(e, key):
    ...
    cmd = f'ffmpeg -i "{dl}" -preset fast -c:v libx265 -crf 25 -map 0:v -c:a opus -ac 2 -b:a 70k -map 0:a -c:s copy -map 0:s? -metadata title="Tamil Fusion | t.me/TamilFusion1" -metadata "Encoded/Mixed By"="TxR Encodes | t.me/TamilFusion1" -metadata:s:v title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:a title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:s title="Tamil Fusion | t.me/TamilFusion1" "{out}" -y'
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
