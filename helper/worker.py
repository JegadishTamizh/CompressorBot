...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -preset fast -c:v libx265 -crf 26 -map 0:v -c:a libopus -b:a 70k -ac 2 -map 0:a -c:s copy -map 0:s? -metadata title="Tamil Fusion | t.me/TamilFusion1" -metadata Encoded_by="TxR Encodes | t.me/TamilFusion1" -metadata:s:v title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:a title="Tamil Fusion | t.me/TamilFusion1" -metadata:s:s title="Tamil Fusion | t.me/TamilFusion1" "{out}" -y'
        process = await asyncio.create_subprocess_shell(
